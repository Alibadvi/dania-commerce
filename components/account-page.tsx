...
export function AccountPage() {
  ...
  if (dashboard) {
    ...
    return (
      <main className="account-page customer-account-page !bg-[#f7f1e8] !px-4 !py-10 md:!py-14 [&_.customer-dashboard]:!max-w-[1180px] [&_.customer-dashboard-head]:!rounded-[28px] [&_.customer-dashboard-head]:!bg-[#102b49] [&_.customer-dashboard-head]:!p-6 md:[&_.customer-dashboard-head]:!p-9 [&_.customer-dashboard-head]:!text-white [&_.customer-dashboard-head_.eyebrow]:!text-[#ffc83e] [&_.customer-dashboard-head_p]:!text-slate-300 [&_.customer-dashboard-head_.button]:!bg-[#ff6b68] [&_.account-dashboard-grid]:!gap-5 [&_.account-dashboard-nav]:!rounded-3xl [&_.account-dashboard-nav]:!border-[#102b49]/10 [&_.account-dashboard-nav]:!shadow-none [&_.account-dashboard-nav_nav_button]:!rounded-xl [&_.account-dashboard-nav_nav_button.active]:!bg-[#102b49] [&_.account-avatar>svg]:!bg-[#ffc83e]/20 [&_.account-avatar>svg]:!text-[#102b49] [&_.account-stat-grid>*]:!rounded-2xl [&_.account-stat-grid>*]:!border-[#102b49]/10 [&_.account-stat-grid>*]:!shadow-none [&_.account-stat-grid_strong]:!text-[#ff6b68] [&_.dashboard-card]:!rounded-3xl [&_.dashboard-card]:!border-[#102b49]/10 [&_.dashboard-card]:!shadow-none [&_.dashboard-form-grid_input]:!min-h-12 [&_.dashboard-form-grid_input]:!rounded-xl [&_.dashboard-form-grid_input]:!border-[#102b49]/15 [&_.dashboard-form-grid_textarea]:!rounded-xl [&_.dashboard-form-grid_textarea]:!border-[#102b49]/15 [&_.dashboard-form-grid_input:focus]:!border-[#315dff] [&_.dashboard-form-grid_input:focus]:!ring-4 [&_.dashboard-form-grid_input:focus]:!ring-[#315dff]/10 [&_.address-card]:!rounded-2xl [&_.address-card]:!border-[#102b49]/10 [&_.account-order]:!rounded-2xl [&_.account-order]:!border-[#102b49]/10">
  ...
  return <main className="account-page !bg-[#f7f1e8] !px-3 !py-8 md:!py-14"><section className="account-shell !min-h-[560px] !max-w-[1040px] !rounded-[32px] !border !border-[#102b49]/10 !shadow-[0_28px_80px_rgba(16,43,73,.12)]"><div className="account-intro !bg-[radial-gradient(circle_at_18%_18%,rgba(255,107,104,.25),transparent_34%),#102b49] [&_.eyebrow]:!text-[#ffc83e]"><span className="eyebrow">حساب دانیا</span><h1>همه‌چیز برای<br/>قدم بعدی.</h1><p>سفارش‌ها و اطلاعات خریدت در یک جای امن می‌مانند.</p><div className="account-security"><ShieldIcon/><span><strong>ورود امن</strong>نشست حساب فقط در کوکی محافظت‌شده نگهداری می‌شود.</span></div></div><div className="account-panel !bg-white [&_.account-tabs]:!rounded-full [&_.account-tabs]:!bg-[#f7f1e8] [&_.account-tabs_button]:!rounded-full [&_.account-tabs_button.active]:!bg-[#102b49] [&_.account-tabs_button.active]:!text-white [&_.account-form_input]:!min-h-12 [&_.account-form_input]:!rounded-xl [&_.account-form_input]:!border-[#102b49]/15 [&_.account-form_input:focus]:!border-[#315dff] [&_.account-form_input:focus]:!ring-4 [&_.account-form_input:focus]:!ring-[#315dff]/10 [&_.account-form_.button]:!min-h-12 [&_.account-form_.button]:!rounded-full [&_.account-form_.button]:!bg-[#ff6b68]"><div className="account-tabs" role="tablist" aria-label="ورود یا عضویت"><button type="button" role="tab" aria-selected={mode === "login"} className={mode === "login" ? "active" : ""} onClick={() => { setMode("login"); setError(null); }}>ورود</button><button type="button" role="tab" aria-selected={mode === "register"} className={mode === "register" ? "active" : ""} onClick={() => { setMode("register"); setError(null); }}>ساخت حساب</button></div><form className="account-form" onSubmit={submitAuth}>{mode === "register" && <div className="name-fields"><label>نام<input required maxLength={60} autoComplete="given-name" name="firstName"/></label><label>نام خانوادگی<input required maxLength={80} autoComplete="family-name" name="lastName"/></label></div>}<label>ایمیل<input required type="email" maxLength={254} inputMode="email" autoComplete="email" name="emailAddress" dir="ltr"/></label><label>رمز عبور<input required type="password" minLength={8} maxLength={72} autoComplete={mode === "login" ? "current-password" : "new-password"} name="password" dir="ltr"/></label><p className="password-hint">حداقل ۸ کاراکتر</p>{error && <p className="form-message error" role="alert">{error}</p>}{notice && <p className="form-message success" role="status">{notice}</p>}<button className="button primary wide" disabled={busy} type="submit">{busy ? "کمی صبر کنید…" : mode === "login" ? "ورود به حساب" : "ساخت حساب"}</button></form></div></section></main>;
}
