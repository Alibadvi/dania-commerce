...
import type { CartOrder, CheckoutInput, CheckoutResult, ShippingMethod } from "@/lib/commerce-types";
...
export function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/commerce?resource=shipping", { credentials: "same-origin", cache: "no-store" })
      ...
      .catch((reason: unknown) => {
        if (!cancelled) setError(reason instanceof Error ? reason.message : "روش‌های ارسال دریافت نشد.");
      })
      .finally(() => {
  ...
  async function submitCheckout(event: React.FormEvent<HTMLFormElement>) {
    ...
    const checkout: CheckoutInput = {
      ...
      streetLine1: String(form.get("streetLine1") ?? ""),
      postalCode: String(form.get("postalCode") ?? ""),
      orderNote: String(form.get("orderNote") ?? ""),
      shippingMethodId,
    };
    try {
      const response = await fetch("/api/commerce", {
        ...
        body: JSON.stringify({ action: "checkout.place", checkout }),
      });
  ...
  async function submitCoupon() {
    ...
    try {
      ...
      setCouponMessage("کد تخفیف روی سبد اعمال شد.");
    } catch (reason) {
      setCouponMessage(reason instanceof Error ? reason.message : "کد تخفیف اعمال نشد.");
    }
  }
  ...
  return <main className="checkout-page"><div className="container checkout-heading"><span className="eyebrow">مرحله آخر</span><h1>تکمیل سفارش</h1></div><div className="container checkout-layout"><form className="checkout-form" onSubmit={submitCheckout}>{error && <div className="commerce-alert" role="alert">{error}</div>}<section><div className="form-section-heading"><b>۱</b><div><h2>مشخصات تحویل‌گیرنده</h2><p>اطلاعات برای ثبت سفارش در Vendure استفاده می‌شود.</p></div></div><div className="form-grid"><label>نام و نام خانوادگی<input name="fullName" required autoComplete="name" maxLength={100} placeholder="مثلاً علی رضایی"/></label><label>شماره موبایل<input name="phoneNumber" required autoComplete="tel" inputMode="tel" dir="ltr" maxLength={20} placeholder="09120000000"/></label><label className="full">ایمیل<input name="emailAddress" required autoComplete="email" inputMode="email" dir="ltr" maxLength={254} placeholder="you@example.com"/></label><label>استان<input name="province" required autoComplete="address-level1" maxLength={80} placeholder="تهران"/></label><label>شهر<input name="city" required autoComplete="address-level2" maxLength={80} placeholder="تهران"/></label><label className="full">آدرس کامل<textarea name="streetLine1" required autoComplete="street-address" maxLength={300} rows={3} placeholder="خیابان، کوچه، پلاک و واحد"/></label><label>کد پستی<input name="postalCode" required autoComplete="postal-code" inputMode="numeric" dir="ltr" maxLength={20} placeholder="10 رقمی"/></label><label>توضیحات سفارش <input name="orderNote" maxLength={500} placeholder="اختیاری"/></label></div></section><section><div className="form-section-heading"><b>۲</b><div><h2>روش ارسال</h2><p>هزینه‌ها مستقیماً توسط موتور سفارش محاسبه می‌شوند.</p></div></div><div className="delivery-options">{loadingShipping && <p>در حال دریافت روش‌های ارسال…</p>}{!loadingShipping && shippingMethods.map((method) => <label key={method.id} className={shippingMethodId === method.id ? "active" : ""}><input type="radio" name="shippingMethod" value={method.id} checked={shippingMethodId === method.id} onChange={() => setShippingMethodId(method.id)}/><span><strong>{method.name}</strong><small>{method.description}</small></span><b>{method.priceWithTax ? `${formatPrice(method.priceWithTax)} تومان` : "رایگان"}</b></label>)}</div></section><section><div className="form-section-heading"><b>۳</b><div><h2>پرداخت</h2><p>محیط محلی از پرداخت آزمایشی استفاده می‌کند؛ production فقط با درگاه واقعی تکمیل می‌شود.</p></div></div><div className="payment-placeholder"><ShieldIcon/><span><strong>پرداخت امن</strong><small>نشست سبد در کوکی HttpOnly نگه‌داری می‌شود</small></span></div></section><button className="button primary wide checkout-submit" type="submit" disabled={!cart.length || submitting || loadingShipping || !shippingMethodId}>{submitting ? "در حال ثبت سفارش…" : cart.length ? `ثبت سفارش ${formatPrice(payable)} تومان` : "سبد خرید خالی است"}</button></form><aside className="order-summary"><h2>خلاصه سفارش</h2>{cartBusy && !order ? <div className="summary-empty"><p>در حال همگام‌سازی سبد…</p></div> : cart.length === 0 ? <div className="summary-empty"><p>هنوز محصولی انتخاب نکرده‌ای.</p><Link href="/shop">رفتن به فروشگاه</Link></div> : cart.map((line) => <div className="summary-line" key={line.id}><div className={`summary-thumb product-image ${line.imagePosition}`}/><div><strong>{line.productName}</strong><span>سایز {line.size.toLocaleString("fa-IR")} · {line.quantity.toLocaleString("fa-IR")} عدد</span></div><b>{formatPrice(line.linePrice)}</b></div>)}<div className="summary-totals"><p><span>جمع محصولات</span><b>{formatPrice(order?.subTotal ?? 0)} تومان</b></p><p><span>هزینه ارسال</span><b>{selectedShipping?.priceWithTax ? `${formatPrice(selectedShipping.priceWithTax)} تومان` : "رایگان"}</b></p><p className="grand-total"><span>مبلغ نهایی</span><b>{formatPrice(payable)} تومان</b></p></div><div className="promo-form"><input value={couponCode} onChange={(event) => setCouponCode(event.target.value)} dir="ltr" maxLength={32} placeholder="کد تخفیف"/><button type="button" disabled={!couponCode.trim() || cartBusy} onClick={() => void submitCoupon()}>اعمال</button></div>{couponMessage && <p className="coupon-message" role="status">{couponMessage}</p>}</aside></div></main>;
}
